...
python3 ~/github/SquiggleKit/fast5_fetcher_multi.py -q $cat_readfile  \
...
cat `find $consensus_ref -name "*${uo}*" | sort | head -n 1` | tee $4_current_reference.fa >> $4_tombo_reference.fa;
...
tombo detect_modifications de_novo --rna --fast5-basedirs $raw_dir --statistics-file-basename $4

echo 'tombo text output'
tombo text_output browser_files --browser-file-basename $4 --file-types dampened_fraction coverage \
--statistics-filename $4.tombo.stats --fast5-basedirs $raw_dir
